...
# In[1]:
import ajay_nucleus1 as nucleus1
...
def image2mask(image, mask, w, h):
    ...
    for row in range(0, w - sz - 1, sz):
        ...
        for col in range(0, h - sz - 1, sz):
            ...
            tileM = mask[row:row + sz, col:col + sz]
            if np.sum(tileM): 
...
filePath = '/nfs/data/main/M33/fetal_hippocampus/'
# In[13]:
...
for files in fileList1:
    ...
    if files.replace('\n', '') not in fileList2:
        ...
        # imwrite_fast(os.path.join(outDir, files.replace('_lossy.jp2', '_mask.jp2')), mask)
        
        # del mask
        # imwrite_fast(os.path.join(outDir, files.replace('_lossy.jp2', '_mask.jp2')), image)
        ...
        del op
        # imwrite_fast(os.path.join(outDir, files.replace('_lossy.jp2', '.jp2')), opC)
        # del opC